; MoneroV Hayek GUI Wallet Installer for Windows
...
; MoneroV GUI wallet
Source: "bin\monerov-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "monerov-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Monero CLI wallet
Source: "bin\monerov-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero wallet RPC interface implementation
Source: "bin\monerov-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero daemon
Source: "bin\monerovd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "monerov-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Monero blockchain utilities
Source: "bin\monerov-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\monerov-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\monerov-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "monerov-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Monero daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\monerovd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\monerov-daemon.bat'), s, false); 
  end;
end;
...
Name: "{group}\Utilities\Monero Daemon"; Filename: "{app}\monerovd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\monerov-wallet-cli.exe"; WorkingDir: "{userdocs}\Monero\wallets"
...
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\monerov-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\monerov-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM monerovd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\monerov-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\monerov-project\monerov-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
